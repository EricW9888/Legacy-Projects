...
class fplayer(pygame.sprite.Sprite):
  def __init__(self):
    pygame.sprite.Sprite.__init__(self)
    self.fishx = 300
    self.fishy = 350
    self.fish = pygame.image.load('images/ffish.png').convert()
    self.image = self.fish
    self.rect = self.fish.get_rect()

class fplayer2(pygame.sprite.Sprite):
  def __init__(self):
    pygame.sprite.Sprite.__init__(self)
    self.fish2x = 300
    self.fish2y = 350
    self.fish2 = pygame.image.load('images/ffish2.png').convert()
    self.image = self.fish2
    self.rect = self.fish2.get_rect()

class fplayer3(pygame.sprite.Sprite):
  def __init__(self):
    pygame.sprite.Sprite.__init__(self)
    self.fish3x = 300
    self.fish3y = 350
    self.fish3 = pygame.image.load('images/ffish3.png').convert()
    self.image = self.fish3
    self.rect = self.fish3.get_rect()
